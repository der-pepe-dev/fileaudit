...
- C#/.NET 10 (CLI + library, cross-platform, no UI).
...
- Stateless: the JSONL report is the only artifact — no manifests, hashes, or DB.
- Read-only and reporting-only: NEVER write to scanned files.
- No repair, no dedup, no background services, no UI in v0.1.
- Layering: Abstractions (DTOs/IVerifier) / Checks (pure, stream-in/finding-out) / Core (engine + reporter) / Plugins / Cli.
- Checks must be pure: no console, no process spawning, no env dependence.
- Verifiers emit DefectEvent instead of throwing; CanVerify must be cheap; respect cancellation.
- Stable contracts (docs/contract.md, matching_rules.md, verifiers.md) must not drift — update them in the same change.
- See docs/design-principles.md and docs/engine-rules.md for the full rules.
